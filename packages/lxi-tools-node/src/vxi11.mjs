...
export async function getVxi11Port(host, timeout = 3000, portmapperPort = 111) {
	const socket = await tcpConnect(host, portmapperPort, timeout);
	try {
...
function buildCreateLink(clientId, lockTimeout, device) {
...
export class Vxi11Session {
	...
	async close() {
		try {
			...
			const payload = await this._rpcCall(DESTROY_LINK, payloadBuffer);
			parseDestroyLinkReply(payload);
		} catch {
...
export async function vxi11Connect(host, port, name, timeout, _portmapperPort) {
	// Resolve VXI-11 port via portmapper if port is 0 or 111
	let vxi11Port = port;
	if (!port || port === 111) {
		vxi11Port = await getVxi11Port(host, timeout, _portmapperPort);
	...
	try {
		...
		const payloadBuffer = buildCreateLink(clientId, timeout, deviceName);
