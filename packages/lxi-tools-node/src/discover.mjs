...
export async function discoverVxi11(options = {}) {
	...
	const udpPort = options._port ?? 111;
	const vxiPort = options._vxiPort ?? 0;
	...
	return new Promise((resolve) => {
		...
		socket.bind(0, () => {
			...
			const targets = options._broadcastTargets ?? broadcastTargets;
			for (const target of targets) {
				if (onBroadcast) onBroadcast(target.name);
				socket.send(buffer, 0, buffer.length, udpPort, target.broadcast);
			}
...
export async function discoverMdns(options = {}) {
	...
	const mdnsAddr = options._mdnsAddr ?? MDNS_ADDR;
	const mdnsPort = options._mdnsPort ?? MDNS_PORT;
...
export function parseMdnsResponse(buffer) {
	const results = [];
